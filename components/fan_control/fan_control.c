...
void cooler_pwm_set_power(uint8_t power) {
    ...
    // 将百分比映射到 8 位占空比值
    uint32_t duty = (power * ((1 << 8) - 1)) / 100;
    // 设置占空比并更新
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, cooler_channel, duty);
    ledc_update_duty(LEDC_HIGH_SPEED_MODE, cooler_channel);
}
